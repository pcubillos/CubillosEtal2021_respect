...
for j,i in enumerate(plot_models[k]):
    ...
    ax = mc3.plots.subplotter(rect, margin, j+1, nx=nx, ny=ny)
    di_eb = ax.errorbar(
...
for m in range(nmol):
    ...
    for i in range(nphase):
        ...
        axes[i].plot(0, np.log10(true_q[i,m]), 'o', ms=3, c=rc[m])
...
plt.savefig(f'plots/model_WASP43b_{model_names[k][9:13]}_retrieved_spectra_temp_abundances.pdf')
